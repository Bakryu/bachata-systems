...
import { Typography, Container, ScrollReveal } from '@/components/ui';
import {
  FaUsers,
  FaBolt,
  FaLock,
...
const getMetricIcon = (iconName: string) => {
  ...
  return iconMap[iconName] || <HiSparkles className="text-4xl text-accent-yellow" />;
};
...
interface Project {
  ...
  description: string;
  category: string;
  technologies: Array<{ icon: React.ReactNode; name: string }>;
...
// Variant C: Asymmetric Modern Layout with Bold Typography
        ...
        {project.video && (
          <ScrollReveal delay={0.2}>
            <div className="relative mb-12 lg:mb-32">
              <div className="absolute -inset-4 bg-gradient-to-r from-accent-yellow via-accent-blue to-accent-purple rounded-3xl opacity-20 blur-2xl" />
              <div className="relative rounded-2xl overflow-hidden border-4 border-accent-yellow/30">
                <video className="w-full h-auto" autoPlay loop muted playsInline>
                  <source src={project.video.desktop} type="video/mp4" />
                </video>
              </div>
            </div>
          </ScrollReveal>
